Pod::Spec.new do |s|
  ...
  lib_url = "https://stash.truex.com/integration/TruexAdRenderer-tvOS-v#{s.version}"
  ...
  # Innovid has varying pods depending on which Xcode we are using.
  innovid_xcode_version = "xcode12.0.0"
  if XCODE_MAJOR >= 13 || XCODE_MAJOR == 12 && XCODE_MINOR >= 5
    innovid_xcode_version = "xcode12.5.1"
  elsif XCODE_MAJOR <= 11
    # legacy build system does not support Innovid Xcode 12+
    innovid_xcode_version = 'xcode11.4.0'
  end
  log.("using InnovidRenderer_#{innovid_xcode_version}")
  s.dependency "InnovidAdRenderer_#{innovid_xcode_version}", '~> 1.6.0' # pod version is good for all xcode variations
